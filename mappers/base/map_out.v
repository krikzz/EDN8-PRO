	...
	wire [7:0]map_cpu_dout, map_ppu_dout, ss_rdat, eep_ram_di;
	wire ciram_a10, ciram_ce, ram_we, chr_we, irq, chr_oe, prg_oe, chr_ce, ram_ce, rom_ce, pwm, map_cpu_oe, rom_we, map_ppu_oe, mir_4sc, mem_dma, map_led, bus_conflicts, eep_on;
	
	
	assign map_out[`BW_MAP_OUT-1:0] = 
	{
		 eep_ram_di[7:0], eep_on, bus_conflicts, map_led, mem_dma, mir_4sc, map_ppu_oe, map_cpu_oe, !pwm, 
	...
	wire sync_m2;
	wire prg_oe_o = eep_oe | (sync_m2 ? (m2 & prg_oe & !sys_rst) : prg_oe);
	wire ram_we_o = eep_we | (sync_m2 ? (m2 & ram_we & !sys_rst & m3) : ram_we);
	wire ram_ce_o = eep_ce | (sync_m2 ? (m2 & ram_ce & !sys_rst & !cfg_prg_ram_off) : ram_ce);//?
	wire rom_we_o = sync_m2 ? (m2 & rom_we & !sys_rst & m3) : rom_we;
	wire rom_ce_o = sync_m2 ? (m2 & rom_ce & !sys_rst & !eep_ce) : rom_ce & !eep_ce;
	...
	wire [17:0]srm_addr;
	wire [17:0]srm_addr_msk = srm_mask_off ? srm_addr[17:0] : {(srm_addr[17:7] & srm_msk[10:0]), srm_addr[6:0]};
	...
	wire [22:0]prg_addr_std = rom_ce & !eep_ce ? prg_addr[22:0] : srm_addr_msk[17:0];
	wire [22:0]chr_addr_std = chr_addr[22:0];
	
	wire [22:0]prg_addr_msk = rom_ce & !eep_ce ? {(prg_addr[22:13] & prg_msk[9:0]), prg_addr[12:0]} : srm_addr_msk[17:0];
	wire [22:0]chr_addr_msk = {(chr_addr[22:13] & chr_msk[9:0]), chr_addr[12:0]};
	
	
	wire [1:0]fds_msk = prg_msk[4:3];
	
	wire eep_ce = (eep_we | eep_oe) & !cfg_prg_ram_off;
	wire eep_we, eep_oe;