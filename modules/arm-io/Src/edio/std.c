...
void str_append_hex8(u8 num, u8 *dst) {
    ...
    val_hi += (val_hi < 10 ? '0' : '7');
    val_lo += (val_lo < 10 ? '0' : '7');
...
void crc16SD_SW(void *src, u16 *crc_out) {
    ...
    for (i = 0; i < 128; i++) {
        ...
        tmp1 = crc_table[0];
        crc_table[0] = (crc_16_table[(tmp1 >> 8) ^ val[0]]) ^ (tmp1 << 8);

        tmp1 = crc_table[1];
        crc_table[1] = (crc_16_table[(tmp1 >> 8) ^ val[1]]) ^ (tmp1 << 8);

        tmp1 = crc_table[2];
        crc_table[2] = (crc_16_table[(tmp1 >> 8) ^ val[2]]) ^ (tmp1 << 8);

        tmp1 = crc_table[3];
        crc_table[3] = (crc_16_table[(tmp1 >> 8) ^ val[3]]) ^ (tmp1 << 8);
